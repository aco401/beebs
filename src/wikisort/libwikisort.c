...
bool wikisort_TestingMostlyEqual(Test item1, Test item2) { return (item1.value < item2.value); }
...
void wikisort_InsertionSort(Test array[], const Range range, const Comparison compare) {
	long i;
...
void WikiSort(Test array[], const long size, const Comparison compare) {
	...
	if (size <= 32) {
		wikisort_InsertionSort(array, wikisort_MakeRange(0, size), compare);
		return;
	}
	...
	while (decimal < size) {
		...
		wikisort_InsertionSort(array, wikisort_MakeRange(start, end), compare);
	}
	...
	for (merge_size = 16; merge_size < power_of_two; merge_size += merge_size) {
		...
		if (wikisort_Range_length(level1) > 0) {
			...
			wikisort_InsertionSort(array, level2, compare);
...
int beebs_wikisort_benchmark() {
	...
	Comparison compare = wikisort_TestingMostlyEqual;
